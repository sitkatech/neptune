...
public abstract class ScheduledBackgroundJobBase<T>
{
    ...
    protected readonly HippocampDbContext DbContext;
    protected readonly HippocampConfiguration HippocampConfiguration;
    ...
    protected ScheduledBackgroundJobBase(string jobName, ILogger<T> logger, IWebHostEnvironment webHostEnvironment, HippocampDbContext dbContext, IOptions<HippocampConfiguration> hippocampConfiguration, SitkaSmtpClientService sitkaSmtpClient)
    {
        _jobName = jobName;
        Logger = logger;
        _webHostEnvironment = webHostEnvironment;
        DbContext = dbContext;
        HippocampConfiguration = hippocampConfiguration.Value;
    ...
    public void RunJob(IJobCancellationToken token)
    {
        lock (ScheduledBackgroundJobLock)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Subject = $"Hippocamp Hangfire Job Failed: Job {_jobName}",
                    Body = $"Details: <br /><br />{ex.Message}",
                    IsBodyHtml = true
                };

                mailMessage.To.Add(new MailAddress(HippocampConfiguration.SITKA_EMAIL_REDIRECT));
