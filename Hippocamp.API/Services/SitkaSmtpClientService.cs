...
namespace Hippocamp.API.Services
{
    ...
    {
        private readonly HippocampConfiguration _hippocampConfiguration;

        //private static readonly ILog _logger = LogManager.GetLogger(typeof(SitkaSmtpClient));

        public SitkaSmtpClientService(HippocampConfiguration hippocampConfiguration)
        {
            _hippocampConfiguration = hippocampConfiguration;
        }

        /// <summary>
        /// Sends an email including mock mode and address redirection  <see cref="HippocampConfiguration.SITKA_EMAIL_REDIRECT"/>, then calls onward to <see cref="SendDirectly"/>
        /// </summary>
        ...
        /// <summary>
        /// Sends an email message at a lower level than <see cref="Send"/>, skipping mock mode and address redirection  <see cref="HippocampConfiguration.SITKA_EMAIL_REDIRECT"/>
        /// </summary>
        /// <param name="mailMessage"></param>
        public void SendDirectly(MailMessage mailMessage)
        {
            //if (!string.IsNullOrWhiteSpace(HippocampConfiguration.MailLogBcc))
            //{
            ...
            var smtpClient = new SmtpClient(_hippocampConfiguration.SMTP_HOST, _hippocampConfiguration.SMTP_PORT);
            smtpClient.Send(mailMessage);
            //_logger.Info($"Email sent to SMTP server \"{smtpClient.Host}\", Details:\r\n{humanReadableDisplayOfMessage}");
        }

        /// <summary>
        /// Alter message TO, CC, BCC if the setting <see cref="HippocampConfiguration.SITKA_EMAIL_REDIRECT"/> is set
        ...
        {
            var redirectEmail = _hippocampConfiguration.SITKA_EMAIL_REDIRECT;
...
Respectfully, the {_hippocampConfiguration.PlatformLongName} team
<br /><br />
***
<br /><br />
You have received this email because you are a registered user of the {_hippocampConfiguration.PlatformLongName}. 
<br /><br />
<a href=""mailto:{_hippocampConfiguration.LeadOrganizationEmail}"">{_hippocampConfiguration.LeadOrganizationEmail}</a>";
...
***
<br /><br />
You have received this email because you are assigned to receive support notifications within the {_hippocampConfiguration.PlatformLongName}. 
<br /><br />
<a href=""mailto:{_hippocampConfiguration.LeadOrganizationEmail}"">{_hippocampConfiguration.LeadOrganizationEmail}</a>";
        ...
        {
            return new MailAddress("donotreply@sitkatech.com", $"{_hippocampConfiguration.PlatformLongName}");
        }
