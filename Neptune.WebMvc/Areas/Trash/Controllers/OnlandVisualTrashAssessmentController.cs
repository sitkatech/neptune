...
using Neptune.WebMvc.Areas.Trash.Views;
...
using Index = Neptune.WebMvc.Areas.Trash.Views.OnlandVisualTrashAssessment.Index;
using IndexViewData = Neptune.WebMvc.Areas.Trash.Views.OnlandVisualTrashAssessment.IndexViewData;
...
namespace Neptune.WebMvc.Areas.Trash.Controllers
{
    public class OnlandVisualTrashAssessmentController : NeptuneBaseController<OnlandVisualTrashAssessmentController>
    {
        ...
        public GridJsonNetJObjectResult<OnlandVisualTrashAssessment> OVTAGridJsonData()
        {
            ...
            var onlandVisualTrashAssessments = _dbContext.OnlandVisualTrashAssessments
        ...
        public GridJsonNetJObjectResult<OnlandVisualTrashAssessmentArea> OnlandVisualTrashAssessmentAreaGridData()
        {
            ...
            var onlandVisualTrashAssessmentAreas = _dbContext.OnlandVisualTrashAssessmentAreas.AsNoTracking().Where(x => stormwaterJurisdictionIDsPersonCanView.Contains(x.StormwaterJurisdictionID)).ToList();
        ...
        private List<OnlandVisualTrashAssessment> GetOVTAsAndGridSpec(out OnlandVisualTrashAssessmentIndexGridSpec gridSpec, Person currentPerson, OnlandVisualTrashAssessmentArea onlandVisualTrashAssessmentArea)
        {
            ...
            return _dbContext.OnlandVisualTrashAssessments.Where(x => x.OnlandVisualTrashAssessmentAreaID == onlandVisualTrashAssessmentArea.OnlandVisualTrashAssessmentAreaID).OrderByDescending(x => x.CompletedDate).ToList();
        }

        [HttpGet("{ovtaID}")]
        [NeptuneViewFeature]
        public ViewResult Instructions([FromRoute] int? ovtaID) // route "overloaded" so we can handle revisiting and starting anew with the same route.
        {
            var viewModel = new InstructionsViewModel();
            var onlandVisualTrashAssessment = ovtaID.HasValue
                ? OnlandVisualTrashAssessments.GetByID(_dbContext, ovtaID.Value)
                : null;
        ...
        [HttpGet("{onlandVisualTrashAssessmentPrimaryKey}")]
        [NeptuneViewFeature]
        public ViewResult InitiateOVTA([FromRoute] int? onlandVisualTrashAssessmentPrimaryKey)
        {
            var onlandVisualTrashAssessment = onlandVisualTrashAssessmentPrimaryKey.HasValue
                ? OnlandVisualTrashAssessments.GetByID(_dbContext, onlandVisualTrashAssessmentPrimaryKey.Value)
                : null;
        ...
        [NeptuneViewFeature]
        public async Task<ActionResult> InitiateOVTA([FromRoute] int? onlandVisualTrashAssessmentPrimaryKey, InitiateOVTAViewModel viewModel)
        {
            var onlandVisualTrashAssessment = onlandVisualTrashAssessmentPrimaryKey.HasValue
                ? OnlandVisualTrashAssessments.GetByIDWithChangeTracking(_dbContext, onlandVisualTrashAssessmentPrimaryKey.Value)
                : null;
