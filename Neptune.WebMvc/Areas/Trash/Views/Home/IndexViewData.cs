...
using Neptune.WebMvc.Areas.Trash.Views.Shared;
using Neptune.WebMvc.Common;
using Neptune.WebMvc.Controllers;
using Neptune.WebMvc.Models;
using Neptune.WebMvc.Views.Shared;
using NetTopologySuite.Features;
using TrashGeneratingUnitController = Neptune.WebMvc.Areas.Trash.Controllers.TrashGeneratingUnitController;
...
namespace Neptune.WebMvc.Areas.Trash.Views.Home
{
    public class IndexViewData : TrashModuleViewData
    {
        ...
        public IndexViewData(HttpContext httpContext, LinkGenerator linkGenerator, Person currentPerson, WebConfiguration webConfiguration, NeptunePage neptunePage, MapInitJson ovtaBasedMapInitJson,
            ...
            FeatureCollection geoJsonForJurisdictions, NeptunePage neptunePageTrashModuleProgramOverview) : base(httpContext, linkGenerator, currentPerson, webConfiguration, neptunePage)
        {
            ...
            BeginOVTAUrl = SitkaRoute<OnlandVisualTrashAssessmentController>.BuildUrlFromExpression(linkGenerator, x => x.Instructions(null));
            AddBMPUrl = SitkaRoute<TreatmentBMPController>.BuildUrlFromExpression(linkGenerator, x => x.New());

            //AllOVTAsUrl =
            //    SitkaRoute<OnlandVisualTrashAssessmentController>.BuildAbsoluteUrlHttpsFromExpression(linkGenerator, x => x.Index(),
            //        NeptuneWebConfiguration.CanonicalHostName);
            //FindBMPUrl = SitkaRoute<TreatmentBMPController>.BuildAbsoluteUrlHttpsFromExpression(linkGenerator, x => x.FindABMP(),
            //    NeptuneWebConfiguration.CanonicalHostName);
            //BeginOVTAUrl =
            //    SitkaRoute<OnlandVisualTrashAssessmentController>.BuildAbsoluteUrlHttpsFromExpression(linkGenerator, x =>
            //            x.Instructions(null),
            //        NeptuneWebConfiguration.CanonicalHostName);
            //AddBMPUrl = SitkaRoute<TreatmentBMPController>.BuildAbsoluteUrlHttpsFromExpression(linkGenerator, x => x.New(),
            //    NeptuneWebConfiguration.CanonicalHostName);
