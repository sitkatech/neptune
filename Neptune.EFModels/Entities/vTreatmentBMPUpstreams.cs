...
{
    public static Dictionary<int, Delineation?> ListWithDelineationAsDictionaryIncludeTreatmentBMPType(NeptuneDbContext dbContext)
    {
        var results = new Dictionary<int, Delineation?>();
        var treatmentBMPUpstreams = dbContext.vTreatmentBMPUpstreams.AsNoTracking()
            .ToList();
        var delineations = dbContext.Delineations.Include(x => x.TreatmentBMP).ThenInclude(x => x.TreatmentBMPType).AsNoTracking().ToDictionary(x => x.TreatmentBMPID);
