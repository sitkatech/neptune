...
namespace Neptune.Web.Common.DhtmlWrappers
{
    public class GridSpec<T> : List<ColumnSpec<T>>
    {
        ...
        public string GetColumnTotals(string gridName)
        {
            ...
            return
                $"Sitka.{gridName}.grid.attachFooter(\"{string.Join(",", this.Select(x => (x.GridColumnAggregationType != null) ? x.GridColumnAggregationType.ToString() : string.Empty))}\", [{string.Join(",", this.Select(x => (x.GridColumnAggregationType != null) ? x.GetAlignmentForFooterRow() : "\"\""))}]);";
        }
        ...
        public string BuildGroupingHeader(ColumnHeaderGroupingList groupHeaderTitleAndColSpanTuple)
        {
            return string.Join(",",
                groupHeaderTitleAndColSpanTuple.Select(x =>
                {
                    ...
                    return $"<div style='text-align:center'>{groupTitle}</div>{colSpanPadding}";
                }));
        }
