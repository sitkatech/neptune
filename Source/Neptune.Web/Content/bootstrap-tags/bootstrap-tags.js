...
(function($) {
    ...
    function Tags(context, params) {
        ...
        if($self.options.can_add) {
            ...
            var inputPill = $($self.options.templates.input_pill)
                .append(input)
    ...
    Tags.prototype._addTag = function(pills_list, input, value) {
        ...
        input.qtip("destroy");
        // we need to validate the tag and make sure we have no invalid characters
        var textEntered = value.text;
        if (/[^a-zA-Z0-9-_\s]/.test(textEntered)) {
            var corners = ['right center', 'left center'];
            var flipIt = input.parents('span.right').length > 0;

            // Show the validation error using qTip
            input.qtip({
                content: { text: "Only letters, numbers, spaces, dashes and underscores are allowed." },
                position: {
    ...
    Tags.prototype.addTag = function(pills_list, value) {
        ...
        value = $self.options.onBeforeAdd(value);
