/*-----------------------------------------------------------------------
<copyright file="DatabaseDirectAccessTestFixtureBase.cs" company="Tahoe Regional Planning Agency and Sitka Technology Group">
...
namespace Neptune.Web.UnitTestCommon
{
    ...
    public class DatabaseDirectAccessTestFixtureBase : IDisposable
    {
        ...
        private static DataTable ExecuteSql(string sqlStatements, SqlConnection connection)
        {
            using (var comm = new SqlCommand(sqlStatements, connection))
            {
                ...
                var sqlResult = NeptuneSqlDatabase.ExecuteSqlCommand(comm);
    ...
    public class NeptuneSqlDatabase
    {
        public const string SqlParameterNameForReturnValue = "@RETURN_VALUE";

        private readonly string _sqlDatabaseConnectionString;

        public NeptuneSqlDatabase()
        {
            _sqlDatabaseConnectionString = NeptuneWebConfiguration.DatabaseConnectionString;
        }
