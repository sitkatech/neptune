...
namespace Neptune.Web.Security
{
    ...
    public class TreatmentBMPEditFeature : NeptuneFeatureWithContext, INeptuneBaseFeatureWithContext<TreatmentBMP>
    {
        ...
        {
            var isAssignedToTreatmentBMP = person.CanManageStormwaterJurisdiction(contextModelObject.StormwaterJurisdiction);
            if (!isAssignedToTreatmentBMP)
            {
                return new PermissionCheckResult($"You aren't assigned as an editor for Jurisdiction {contextModelObject.StormwaterJurisdiction.OrganizationDisplayName}");
            }
