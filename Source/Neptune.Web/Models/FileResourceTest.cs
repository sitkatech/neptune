...
where c.DATA_TYPE in ('char','nvarchar','text','ntext','varchar')
      and t.TABLE_TYPE = 'BASE TABLE'  and t.TABLE_NAME not in ('AuditLog', 'NeptunePage', 'ProjectUpdateBatch')
";
            ...
            using (var command = new SqlCommand(findAnyColumnsThatCouldContainUrls))
            {
                ...
                using (var conn = sqlConnection)
                {
                    ...
                    using (var dt = NeptuneSqlDatabase.ExecuteSqlCommand(command).Tables[0])
                    {
