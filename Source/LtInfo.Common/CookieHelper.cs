...
namespace LtInfo.Common
{
    ...
    {
        public static readonly ILog Logger = LogManager.GetLogger(typeof(CookieHelper));

        public static string GetCookieValueOrSetDefault(HttpRequestBase request, string cookieNameAt,
            string defaultValue)
        {
        ...
        public static List<Cookie> GetAllCookiesExceptSessionCookie(CookieCollection cookieCollection)
        {
            ...
            var cookiesToPassOn = allCookiesExceptSessionCookie.Where(x => DoesCookieValueConformToRfc6265(x.Value))
                .ToList();
            ...
            if (nonConformingCookies.Any())
            {
                ...
                var message = string.Format(
                    "While passing on System.Net.Cookie to a web request, filtered out {0} cookies due to non-conformance with RFC6265. Typically this can be ignored unless that cookie is needed in the remote request. If it is a cookie under your control, considering updating it to conform.\r\n{1}",
                    nonConformingCookies.Count, warningMessageList);
        ...
        public static List<HttpCookie> GetAllCookiesExceptSessionCookie(HttpCookieCollection httpCookieCollection)
        {
            ...
            if (nonConformingCookies.Any())
            {
                ...
                var warningMessageList = string.Join("\r\n", warningMessages);
                var message = string.Format(
                    "While passing on System.Web.HttpCookie to a web request, filtered out {0} cookies due to non-conformance with RFC6265. Typically this can be ignored unless that cookie is needed in the remote request. If it is a cookie under your control, considering updating it to conform.\r\n{1}",
                    nonConformingCookies.Count, warningMessageList);
                Logger.WarnFormat(message);
            }
        ...
        /// </summary>
        private const string RegexStringForRfc6265 =
